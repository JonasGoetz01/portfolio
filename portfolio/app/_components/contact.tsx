...
export default function Contact() {
  ...
  function submit(event: FormEvent) {
    ...
    const subject = encodeURIComponent(`${t.contact.subject} ${name || "website"}`);
    ...
    <section className="mt-20 border-t border-line pt-11">
      <div className="grid gap-11 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
        <div className="flex flex-col gap-[14px]">
          <h2 className="text-[26px] font-semibold tracking-[-0.02em]">{t.contact.title}</h2>
          <p className="max-w-[36ch] text-[15px] leading-relaxed text-dim">{t.contact.intro}</p>
      ...
      <p className="mt-14 font-mono text-[11px] text-dim">{t.footer}</p>
    </section>
  );
}
